...
def chrome():
    ...
    # print(f"DELEEEEEEEEEEEEEEEEEEEE|=| {cPath}")
    # browser = Chrome(options=chrome_option)
    try:
        browser = Chrome(options=chrome_option)
    except:
        chrome_driver_path = ChromeDriverManager(path="/opt/airflowcd").install()
        print(f"ChromeDriver downloaded to: {chrome_driver_path}")
        browser = Chrome(service=Service(chrome_driver_path), options=chrome_option)
    return browser
