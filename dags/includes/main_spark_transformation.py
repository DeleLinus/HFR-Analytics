...
GCS_RAW_DATA_PATH = f'gs://kagglexfinal-hfr-bucket/hfr_raw_data/{date_str}/'
GCS_TRANSFORMED_DATA_PATH = 'gs://kagglexfinal-hfr-bucket/hfr_processed_data/'
RAW_DATAFILE = "raw_hfr_data.csv" 
...
def create_spark_session():
    ...
    """
    spark = SparkSession.builder.appName("HFR App").getOrCreate()
    return spark
...
def main():
    ...
    2. Read the data output from scraper.py from GCS
    3. take the data and transform them to tables
    which will then save the mega data to parquet file.
    4. Load the data as parquet files into the second staging area (GCS)
    """
    spark = create_spark_session()
    # just to off logging (default is "WARN")
    spark.sparkContext.setLogLevel("OFF")
    input_data = GCS_RAW_DATA_PATH + RAW_DATAFILE
    mega_df = create_table_keys(mega_data=read_and_transform_data(spark, input_data), output_folder=None)
    # tables
    ...
    for table_name, table_data in tables_dict.items():
        table_data.write.mode("overwrite").parquet(GCS_TRANSFORMED_DATA_PATH+f"{table_name}.parquet")
        print(f"Total number of records in {table_name}: {table_data.count()}")
        print("{0} table staged as {1} in the database".format(table_name, GCS_TRANSFORMED_DATA_PATH+f"{table_name}.parquet"))
